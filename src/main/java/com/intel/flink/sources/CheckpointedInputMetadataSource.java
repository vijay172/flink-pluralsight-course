 ...
 *
 */
public class CheckpointedInputMetadataSource implements SourceFunction<InputMetadata>, ListCheckpointed<Long> {
...
//    private transient InputStream gzipStream;
     ...
     */
    public CheckpointedInputMetadataSource(String dataFilePath) {
     ...
     */
    public CheckpointedInputMetadataSource(String dataFilePath, int servingSpeedFactor) {
     ...
     * @param servingSpeedFactor The serving speed factor by which the logical serving time is adjusted.
     */
    public CheckpointedInputMetadataSource(String dataFilePath, int maxEventDelaySecs, int servingSpeedFactor) {
    ...
    public void run(SourceContext<InputMetadata> sourceContext) throws Exception {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(dataFilePath)))) {
            ...
            while (cnt < eventCnt && reader.ready() && (line = reader.readLine()) != null) {
                cnt++;
                InputMetadata inputMetadata = InputMetadata.fromString(line);
                // extract starting timestamp
                prevDataStartTime = getEventTime(inputMetadata);
            }
            ...
            while (reader.ready() && (line = reader.readLine()) != null) {
                InputMetadata inputMetadata = InputMetadata.fromString(line);
                // extract starting timestamp
                long dataStartTime = getEventTime(inputMetadata);
                ...
                synchronized (lock) {
                    eventCnt++;
                    sourceContext.collectWithTimestamp(inputMetadata, dataStartTime);
